    ...
    unsigned int xid_rep, char **b, int *sz)
{
  ...
  std::map<unsigned int, std::list<reply_t> >::iterator client_it = reply_window_.find(clt_nonce);

  // if there are no replies, add one as a base for min
  if (client_it->second.empty()) {
    client_it->second.push_back(*(new reply_t(0)));
    return NEW;
  }

  std::list<reply_t>::iterator reply_iterator;

  // initialize min
  unsigned int min_xid = client_it->second.begin()->xid;

  for(reply_iterator=client_it->second.begin(); reply_iterator !=client_it->second.end(); reply_iterator++) {
    if (reply_iterator->xid < min_xid)
      min_xid = xid;
  }
  ...
  // at this point we can do some cleanup of old replies
  // Please note I found this delete pattern here:
  // http://stackoverflow.com/questions/596162/
  // can-you-remove-elements-from-a-stdlist-while-iterating-through-it
  std::list<reply_t>::iterator clean_it = client_it->second.begin();
  while (clean_it != client_it->second.end())
  {
      if (clean_it->xid < xid_rep)
      {
          free((*clean_it).buf); // clear the buffe
          client_it->second.erase(clean_it++);
      }
      else
      {
          ++clean_it;  // move along
  ...
  // so an xid can have multiple replies, and only one might be
  // actually marked as done thus we need to scan through all the
  // replies searching for ones where cb_present == true incase we are
  // actually done, instead of immediatly returning INPROGRESS
  rpcs::rpcstate_t state = NEW;
  for(reply_iterator=client_it->second.begin(); reply_iterator !=client_it->second.end(); reply_iterator++)
  {
    // found the current xid
    if (reply_iterator->xid == xid) {
      if (reply_iterator->cb_present) {
  ...
  if (state==NEW)
    client_it->second.push_back(*(new reply_t(xid)));
    ...
    char *b, int sz)
{
  ...
  client_it->second.push_back(*reply);
}
